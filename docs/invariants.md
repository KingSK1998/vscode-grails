...
| Capability adapters (`GormCapabilityAdapter`, `GrailsInjectionCapabilityAdapter`, `TagLibCapabilityAdapter`, `GrailsConfigCapabilityAdapter`) | Stateless capability adapters under `kingsk.grails.lsp.core.capability` | `RequestContext` snapshot, AST nodes, and project classpath evidence | Completion strategies, `DiscoveryService` | Stateless AST derivation per request; `GormCapabilityAdapter.GORM_CLASSPATH_CACHE` is keyed by `ClassLoader` identity and evicted upon dependency refresh/ClassLoader replacement |
