...
The server handles `.gsp` files by converting them into "Virtual Groovy" files before compilation. This preserves line/column mapping while allowing the Groovy AST parser to handle scriptlets (`<% %>`) and expressions (`${ }`).

1. **Conversion**: `GspToGroovyConverter` replaces all non-Groovy characters (HTML) with whitespaces and preserves Groovy code blocks.
2. **Compilation**: The `GrailsCompiler` injects this virtual source into the `CompilationUnit`.
3. **Completions**: The standard `GroovyHelperIntegration` and `ASTVisitor` then work on the virtual AST, providing type inference for domain classes and services as if it were a regular `.groovy` file.
