...
## Current Priority: R2 — Workspace Intelligence & Source-Set Correctness
...
| ---------------------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Grails capability adapters (R2-05)             | ✅ Passing     | Full R2-05 acceptance verified: `GrailsCapabilityAdapterSpec` (7/7), `GrailsCompletionProviderSpec` (6/6). Admitted adapters: `GormCapabilityAdapter` (dynamic finders `findBy*`/`findAllBy*`/`countBy*` with single-prop and multi-prop And/Or boolean combinations up to 5 properties, static methods, instance methods, injected properties `id`/`version`/`errors`, criteria restrictions), `GrailsInjectionCapabilityAdapter` (controller web scopes `params`/`request`/`session`, render parameters, service conventions, `Validateable` command objects), `TagLibCapabilityAdapter` (`g:` and custom tags), and `GrailsConfigCapabilityAdapter` (`grailsApplication.config` conventions). Exact provenance tracking with `CAPABILITY_ADAPTER` origin and `org.grails` artifact coordinates. Negative cases verified: no-capability projects, missing GORM, and immediate invalidation on domain property rename/deletion without application execution. |
| Discover declarations and API origins (R2-04) | ✅ Passing     | Full R2-04 acceptance verified: `DeclarationDiscoverySpec` (6/6), `GroovyCompletionProviderSpec` (7/7). Real fixture signatures for inherited/generic/overloaded members, traits, and Groovy extension modules match resolved declarations with provenance (`DeclarationProvenance`, `ResolvedDeclaration`). Trailing-dot syntax recovery patched in `SourceSetCompilationState`. Authoritative Groovy keywords consolidated in `GrailsUtils.GROOVY_KEYWORDS`. Missing-module negative cases return empty without inventing fallback APIs (`INV-DISC-001/002/003`, `INV-OWN-004/005`). |
| Resolved Gradle project edges (R2-03)          | ✅ Passing     | Full R2-03 acceptance verified: `DependencyInvalidationSpec` (8/8), `PublicationAndLifecycleSpec` (7/7). Replaced name/prefix string heuristics with canonical project directory, buildRoot + gradleProjectPath, and segment-safe classpath/output checks (`isSameOrChildPath`). Explicit `ProjectDependencyEdge` extraction via Tooling API (`IdeaModuleDependency`) and `source-set-export.gradle`. Cross-project dirty propagation follows permitted edges with origin/revision tracking, BFS cycle termination (single visit), and asynchronous non-blocking compilation dispatch without acquiring nested writer locks (`INV-STATE-009`, `INV-OWN-008`). |
| Coherent dependency refresh (R2-02)            | ✅ Passing     | Full R2-02 acceptance verified: `DependencyRefreshSpec` (3/3), `ClasspathAndSourceSetSpec` (11/11), `GradleSyncSpec` (12/12). Coherent dependency add/change/remove without restart; content fingerprint invalidation for changed artifacts at same coordinates (`ArtifactFactCache`, `ProjectCache` v3); stale LKG preservation on failed/cancelled refresh and rejection of late/mismatched builds during project disposal. |
