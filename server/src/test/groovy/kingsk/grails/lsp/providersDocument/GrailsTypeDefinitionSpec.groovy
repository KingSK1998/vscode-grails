...
class GrailsTypeDefinitionSpec extends TypeDefinitionTestSpec {
    ...
    def "should resolve local variable type definition from declaration"() {
        ...
        String content = """class TypeDefinitions {
    public TypeDefinitions() {
        ...
        def locations = getTypeDefinitionLocations(uri, 2, 8)
    ...
    public TypeDefinitions() {
        ...
        def locations = getTypeDefinitionLocations(uri, 3, 8)
    ...
    public TypeDefinitions() {
        ...
        def locations = getTypeDefinitionLocations(uri, 5, 8)
    ...
    TypeDefinitions memberVar
        ...
        def locations = getTypeDefinitionLocations(uri, 1, 4)
    ...
    public TypeDefinitions memberMethod() {
        ...
        def locations = getTypeDefinitionLocations(uri, 1, 11)
    ...
    public TypeDefinitions() {
        ...
        def locations = getTypeDefinitionLocations(uri, 4, 12)
...
class TypeDefinitions {
    public void testMethod() {
        ...
        def locations = getTypeDefinitionLocations(uri, 4, 8)
        ...
        location.uri == uri || location.uri.contains("List") || location.uri.contains("java/util")

        // Verify it's pointing to the correct location within the file
        if (location.uri == uri) {
            location.range.start.line == 4
            location.range.start.character >= 8  // Should be around the "List" token
        }
    }
    ...
    public void testMethod() {
        ...
        def location = locations[0]
        location.uri.contains("String") || location.uri.contains("java/lang") || location.uri == uri
    ...
    def "should resolve interface type definition"() {
        ...
        String content = """interface TestInterface {
    void testMethod()
}
...
class TypeDefinitions implements TestInterface {
    ...
    public void useInterface() {
        ...
        def locations = getTypeDefinitionLocations(uri, 10, 8)
