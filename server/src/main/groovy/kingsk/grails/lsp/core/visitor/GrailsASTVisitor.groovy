...
class GrailsASTVisitor extends ClassCodeVisitorSupport {
    ...
    /**
     * Visit every unit in the compiler (batch for full compile).
     */
    void visitCompilationUnit(GrailsCompiler compiler) {
        if (!compiler) return
        compiler.getSourceUnits().each { visitSourceUnit(it) }
    }

    /**
     * Visit a single source unit, resetting its per-file state.
     */
    void visitSourceUnit(SourceUnit unit) {
        if (!unit) return
        String uri = TextFile.normalizePath(unit.name)
        if (!uri) return
        log.debug("Visiting file: $uri")

        // Reset data for this file
        nodesByURI.remove(uri)
        classNodesByURI.remove(uri)
        nodesByLineIndex.remove(uri)
