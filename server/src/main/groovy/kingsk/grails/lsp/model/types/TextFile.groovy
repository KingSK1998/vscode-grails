		...
		try {
			if (path.startsWith('file:')) {
				...
				try {
					...
					uriObj = URI.create(path.replace(' ', '%20'))
				}
				return new File(uriObj).canonicalPath
			}
			return new File(path).canonicalPath
		} catch (Exception e) {
			try {
				String cleanPath = path.startsWith('file://') ? path.substring(7) : (path.startsWith('file:') ? path.substring(5) : path)
				if (cleanPath.startsWith('/') && cleanPath.length() > 2 && cleanPath.charAt(2) == ':') {
					cleanPath = cleanPath.substring(1)
				}
				return new File(URLDecoder.decode(cleanPath, 'UTF-8')).canonicalPath
			} catch (Exception ex) {
				return path
			}
		}
