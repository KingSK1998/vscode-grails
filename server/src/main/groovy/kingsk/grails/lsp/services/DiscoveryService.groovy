...
/**
 * Service for dynamic discovery of methods, keywords, and completions.
...
class DiscoveryService {
    ...
    private static final Map<String, List<ClassNode>> CLASSNODE_CACHE = new ConcurrentHashMap<>()
     ...
     */
    static List<String> getGroovyDefaultMethods(ErrorService errorService) {
        return KEYWORD_CACHE.computeIfAbsent('groovy_default') {
            try {
                ...
                errorService.handleError("Could not access DefaultGroovyMethods via MetaClass", e, ErrorSource.AST_SERVICE, ErrorSeverity.WARNING)
    ...
    /**
     * Get Collection methods using Groovy utilities
     */
    ...
    /**
     * Get ClassNodes dynamically from standard packages via ClassGraph
     */
    private static List<ClassNode> getClassesFromPackage(String packageName, String uri, List<ClassNode> fallback) {
        ScanResult scanResult = getClassGraphScanResult(uri)
        if (scanResult != null) {
            try {
                def packageInfo = scanResult.getPackageInfo(packageName)
                if (packageInfo != null) {
                    return packageInfo.classInfo
                        .findAll { it.isPublic() && !it.name.contains('$') }
                        .collect { ClassHelper.make(it.name) }
                }
            } catch (Exception e) {
                log.warn("Failed to get classes from package ${packageName} dynamically: ${e.message}")
            }
        }
        return fallback
    }
    ...
    /**
     * Get common Java type ClassNodes using ClassGraph
     */
    static List<ClassNode> getJavaLangTypeNodes(String uri = null) {
        if (CLASSNODE_CACHE.containsKey('java_lang')) {
            return CLASSNODE_CACHE.get('java_lang')
        }
        def fallback = [
        ...
        ScanResult scanResult = getClassGraphScanResult(uri)
        if (!scanResult) return fallback

        def nodes = getClassesFromPackage('java.lang', uri, fallback)
        if (nodes != fallback) CLASSNODE_CACHE.put('java_lang', nodes)
        return nodes
    }
     ...
     */
    static List<String> getJavaLangTypes(String uri = null) {
        return getJavaLangTypeNodes(uri).collect { it.nameWithoutPackage }
    }

    /**
     * Get common Java util type ClassNodes using ClassGraph
     */
    static List<ClassNode> getJavaUtilTypeNodes(String uri = null) {
        if (CLASSNODE_CACHE.containsKey('java_util')) {
            return CLASSNODE_CACHE.get('java_util')
        }
        def fallback = [
        ...
        def nodes = getClassesFromPackage('java.util', uri, fallback)
        if (nodes != fallback) CLASSNODE_CACHE.put('java_util', nodes)
        return nodes
    }
     ...
     */
    static List<String> getJavaUtilTypes(String uri = null) {
        return getJavaUtilTypeNodes(uri).collect { it.nameWithoutPackage }
    }

    /**
     * Get Groovy-specific type ClassNodes using ClassGraph
     */
    static List<ClassNode> getGroovyTypeNodes(String uri = null) {
        if (CLASSNODE_CACHE.containsKey('groovy_lang')) {
            return CLASSNODE_CACHE.get('groovy_lang')
        }
        def fallback = [
        ...
        def nodes = getClassesFromPackage('groovy.lang', uri, fallback)
        if (nodes != fallback) CLASSNODE_CACHE.put('groovy_lang', nodes)
        return nodes
    }
     ...
     */
    static List<ClassNode> getAllTypeNodes(String uri = null) {
        List<ClassNode> allTypes = []
        allTypes.addAll(getPrimitiveTypeNodes())
        allTypes.addAll(getJavaLangTypeNodes(uri))
        allTypes.addAll(getJavaUtilTypeNodes(uri))
        allTypes.addAll(getGroovyTypeNodes(uri))
     ...
     */
    static List<CompletionItem> getAllTypeCompletions(String prefix, String uri = null) {
        return createCompletionItemsFromClassNodes(getAllTypeNodes(uri), prefix, CompletionItemKind.Class)
    }
