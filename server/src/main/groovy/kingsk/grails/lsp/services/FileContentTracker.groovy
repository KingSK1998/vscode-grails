...
class FileContentTracker extends BaseProvider {
    ...
    TextFile getTextFile(String uri) {
        uri ? trackedFiles[TextFile.normalizePath(uri)] : null
    }

    String getContent(String uri) {
        uri ? getTextFile(uri)?.text : null
    }

    int getNextVersion(String uri) {
        uri ? (getTextFile(uri)?.version ?: 0) + 1 : 0
    }
